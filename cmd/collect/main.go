...
func main() {
	...
	case naming.DeviceBitBabbler:
		...
		defer sess.Close()
		if devInfo, ferr := bbusb.FindDevice(); ferr == nil && devInfo != nil && devInfo.FriendlyName != "" {
			log.Printf("using BitBabbler: %s", devInfo.FriendlyName)
		}
		readBits = func(ctx context.Context) ([]byte, error) {
			buf := make([]byte, byteCount)
			n, rerr := sess.ReadRandom(buf)
			if rerr != nil {
				return nil, rerr
			}
			if n < len(buf) {
	...
	for {
		...
		sampleNum++
		// Use compact timestamp without ':' to avoid downstream parsing issues.
